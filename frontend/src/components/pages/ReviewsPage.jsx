...
const ReviewsPage = () => {
    ...
    async function loadReviews() {
        const loadedReviews = (await axios.get(`BACKEND_ADDRESS/reviews/public`)).data;
        ...
        for (let i = (pageNumber - 1) * 12; i < pageNumber * 12; i++) {
            ...
            const imageStr = arrayBufferToBase64(reviewData.reviewImage.data.data);
            ...
            createPageReviews.push(<ReviewHeader 
                ...
                publishDate={reviewData.publishDate > 0 ? `${moment(new Date(reviewData.publishDate)).format("MMM Do YYYY")} | ${stars}` : "Not published"}
        ...
        const pageAmount = Math.ceil(loadedReviews.length / 12);
        const pageDiff = 3;
