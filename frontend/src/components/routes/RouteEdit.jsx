...
const RouteEdit = () => {
    ...
    window.onload = async function loadData() {
        ...
        const route = await axios.get(`BACKEND_ADDRESS/routes/byID/${routeID}`);
        ...
        const foundUser = JSON.parse(loggedInUser);
        if (foundUser._id !== route.data.userID) {
            setCanEdit(false);
            return;
        }

        setInput({ title: route.data.title, description: route.data.description });
        setPoints(route.data.pointsArray);

        document.getElementById("container").style.visibility = "visible";
        document.getElementById("isPublished").checked = route.data.isPublished;
    }
    ...
    async function createReview(event) {
        ...
        axios.post(`BACKEND_ADDRESS/routes/edit`, newRoute);
        setSuccess("Route successfuly saved!");
    }
    ...
    return (
        <>
            {!canEdit ? <div></div> :
            <div className={routeStyles.main} id="container">
                <div className={ `${routeStyles.gallery} center`}>
                    <h2 class="cell" style={{ marginTop:"50px" }}>Edit Route</h2>
                </div>
                ...
                <div className={ `${routeStyles.gallery} center`}>
                    ...
                    <div className={`${routeStyles.createButtonContainer}`}>
                        <button className={`${routeStyles.createButton} cell grow`} onClick={createReview}>Save Route</button>
                    </div>
                </div>
            </div>
        }
